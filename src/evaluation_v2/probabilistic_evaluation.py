...
class ProbabilisticEvaluation(Evaluation):
    def __init__(self, 
        ...
        super().__init__(model, dataset, concept_name, eos_value, growing_num_values, decoder_cat, decoder_num)
    ...
    def sample_suffix(self, prefix, prefix_len, static_inputs, mask, include_model_states):
        ...
        while i <= max_iteration and not eos_predicted():
            ...
            if self.variational_dropout_sampling:
                prediction, (h, c) = self.model.inference(last_event=(list(cat_predictions.values()), list(num_predictions.values())),
                                                          hx=(h,c),
                                                          z=z)
            else:
                prediction, (h, c) = self.model.inference(last_event=(list(cat_predictions.values()), list(num_predictions.values())),
